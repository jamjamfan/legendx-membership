          ...
          {mode === 'demo' ? (
            ...
            <form className="space-y-4" onSubmit={verify}>
              <p className="rounded-lg border border-border/60 bg-muted/40 p-3 text-sm leading-relaxed text-muted-foreground">
                登入碼已經發送咗去 <b className="text-amber-300">{form.email}</b>，請檢查收件箱（冇嘅話睇埋垃圾郵件）。
              </p>
              <div className="space-y-1.5">
                <Label>6 位登入碼</Label>
                <Input inputMode="numeric" maxLength={6} placeholder="123456" value={code} onChange={(e) => { setCode(e.target.value); setError('') }} required className="text-center text-lg tracking-[0.5em]" />
              </div>
              {error && <p className="text-sm text-red-400">{error}</p>}
              <Button type="submit" disabled={busy} className="h-11 w-full bg-gradient-to-r from-amber-400 to-amber-600 font-bold text-[hsl(222,47%,8%)]">
