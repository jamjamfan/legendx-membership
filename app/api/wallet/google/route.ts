...
import { createGoogleWalletSaveUrl } from "@/lib/integrations/google-wallet";
...
export async function GET(request: Request) {
  ...
  const saveUrl = await createGoogleWalletSaveUrl({
    objectSuffix: `class-pass-${user.id}-${sessionId}`,
  ...
  if (!saveUrl) {
    return NextResponse.json({ error: "google_wallet_not_configured" }, { status: 503 });
  }
